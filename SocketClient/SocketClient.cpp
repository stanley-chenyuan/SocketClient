...
BOOL InitInstance(HINSTANCE hInstance, int nCmdShow)
{
   ...
   CSocketClientCode *pClient = new CSocketClientCode();

   pClient->InitializationSocket(2,2);      //初始化socket

   pClient->CreateSocket("192.168.0.27",8800);		// 创建socket
   pClient->ConnectServer();					//链接服务器
   timeSetEvent( 200,0, TimeProc, 0, (UINT)TIME_PERIODIC);//从服务器获得返回的数据
   return TRUE;
}
